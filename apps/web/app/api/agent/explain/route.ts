...
export async function POST(req: NextRequest) {
  try {
    ...
    const { explanation, intentId } = await orchestrator.generateExplain(text.trim());

    await insertAuditEvent({
      id: generateId(),
      eventType: 'AGENT_EXPLAIN_GENERATED',
      walletAddress: typedWallet.address,
      intentId,
      payload: { explanation },
      simulated: explanation.isSimulated,
    });

    return NextResponse.json({ intentId, explanation });
  } catch (err) {
    console.error('[agent/explain]', err);
